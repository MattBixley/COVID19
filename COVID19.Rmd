...
Curves falttening but not dropping, needs to drop. Back to renovation mode with painting the kitchen. Trying not to cut my fingers off making a chopping board from Rimu extracted from the house.  
...
ausdat <- covid19 %>% filter(country == "Australia", log(cases_million) > 0.02)
nzdat <- covid19 %>% filter(country == "New_Zealand", log(cases_million) > 0.02)
plotdata <- covid19 %>% filter(country %in% who, log(cases_million) > 0.02, population > 4800000)
