...
#  mutate_at(vars(state,country), as.factor) %>% 
  str_replace_all(c("US" = "United States", "Taiwan*" = "Taiwan"))
...
# plot nz growth
who <- "New Zealand"
who <- c("New Zealand", "Australia", "US")

p1 <- coronavirus %>% filter(country == who, cumulative_cases > 0) %>% 
  ggplot(aes(x = Date, y = country_cum_cases/population*1000000, colour = country)) +
  geom_point() +
  geom_line()
p1
