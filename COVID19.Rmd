...
covid19 <- read_csv("data/ecdc.csv") %>% 
  ...
  ungroup() %>% 
  mutate(country = ifelse(country == "United_States_of_America", "USA",country)) %>%
  mutate(per_million = round(cum_cases/population*1000000,1)) # per capita
...
# plot nz growth
#who <- "New Zealand"
who <- c("New_Zealand", "Australia", "USA", "Italy")

p1 <- covid19 %>% filter(country %in% who, cum_cases > 100) %>%
  group_by(country) %>% 
...
#----------------------------------------
# Plotting the data
who <- c("New Zealand", "Australia", "USA", "Italy")
p2 <- covid19 %>% filter(country %in% who) %>% 
